...
UINT WINAPI TFunc(LPVOID thParam)
{
	...
	int y1 = 0, y2 = 0, time1 = 0, time2 = 0, y3 = 0, y4 = 0, time3 = 0, time4 = 0;				//プロットするために変換する値

	int by1 = hight1/2, by2 = hight2 / 2, by3 = hight3 / 2, by4 = hight4 / 2;
	int btime1 = width1*0.1, btime2 = width2*0.1, btime3 = width3*0.1, btime4 = width4*0.1;			//前回の値
	...
	while (Flag == TRUE) {
		...
		//データの読み込み
		if (fscanf(fp, "%lf %lf %lf %lf", &data1,&data2,&data3,&data4) == EOF) {
		...
		y1 = data1*5*hight1 / 2 * 0.8;
		y2 = data2*5*hight2 / 2 * 0.8;
